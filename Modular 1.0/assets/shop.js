		...
		images: {
			loadBackgrounds: function() {
				...
				$elementsToLoad.each(function() {
					...
					if(src == '') {
						src = '//cdn.shopify.com/s/files/1/2504/5172/t/5/assets/placeholder-pattern.png?9615822130378923681';
						placeholder = true;
					}
					...
					$(im).on('error', function() {
						$el.css('background-image', 'url(//cdn.shopify.com/s/files/1/2504/5172/t/5/assets/placeholder-pattern.png?9615822130378923681)')
...
var Product = {
	init: function( update ){
		...
		if (!Modernizr.touch && isZoomEnable){
			this.imageZoom.getImage();
		}
	...
	imageZoom: {
		bind: function(){
			Reqs.panZoom(); // grab panZoom library

			$('.js-imageZoom').on('click', function(){
				var $container = $(this).parent('.product-image');

				$container.addClass('zoom--is-loading');

				$container.zoom({
					url: $container.attr('data-bg-src'),
					callback: function(){
						setTimeout(function(){
							$container.addClass('zoom--is-visible');
							$container.removeClass('zoom--is-loading');
						}, 300);

						$container.on('mouseleave touchend', function(){
							$container.addClass('zoom--is-hiding');

							setTimeout(function(){
								$container.removeClass('zoom--is-visible zoom--is-hiding');
		...
		getImage: function(){
			var $image_blocks = $('.product-image');

			$image_blocks.each(function(){
        var $image_block = $(this),
            data_bg = $image_block.attr('data-bg-src') || $image_block.attr('data-image'),
            $image = $image_block.find('.product-image-img'),
            test_img = new Image(); // generate new off screen image

				test_img.src = 'https:'+data_bg;

				$(test_img).load(function(){
					var img_size = {
						w: test_img.width, // get image sizing
						h: test_img.height
					};

          // is image eligible for zoom? 
          if (img_size.w > $image.width()){
            $image_block.append('<div class="js-imageZoom product-image-zoom"></div>');
            Product.imageZoom.bind();
          } 

          // if product_img_handling === 'fill'
          // before fitImages() takes effect, the images show up on screen at the same
          // width as their virtual counterparts we just created.
          $(window).on('fit-images', function(){
            // if the image is larger than it appears within its container, add zoom toggle and bind click event
            if (img_size.w > $image.width()){
              $image_block.append('<div class="js-imageZoom product-image-zoom"></div>');
              Product.imageZoom.bind();
            } 
					...
					// add an data-attribute to each image to watch on screen resize
					$image.attr('data-width', img_size.w);
					Product.imageZoom.watch();
				});
			});
		},
		watch: function(){
			$(window).resize(function(){
				$('.product-image').each(function(){
					...
					// if this image previously was not big enough to zoom
					if (img_width > block_width && !$(this).find('.js-imageZoom').length){
						$(this).append('<div class="js-imageZoom product-image-zoom"></div>');
					}
					// if the image is big enough and .js-imageZoom is present, make sure it's clickable.
					else if (img_width > block_width && $(this).find('.js-imageZoom').length){
						$(this).find('.js-imageZoom').show();
					}
					// otherwise disable zoom
					else {
						$(this).find('.js-imageZoom').hide();
					}
				});

				// bind click event to any new .js-imageZoom toggles
				Product.imageZoom.bind();
			});
		}
	}
